...
public Savetype detect_savetype(ubyte[] rom) {
    ...
    // note that some games will not be present in the lut
    Savetype savetype = lookup_savetype_from_lut(rom);
...
private Savetype lookup_savetype_from_lut(ubyte[] rom) {
    string game_code = cast(string) rom[0xAC..0xAF];

    immutable(Savetype*) savetype = (game_code in savetype_lookup_table);
    if (savetype !is null) {
        return *savetype;
    } else {
...
private immutable Savetype[string] savetype_ids;
private immutable Savetype[string] savetype_lookup_table;

shared static this() {
    // lut derived from https://github.com/fleroviux/NanoBoyAdvance/blob/master/src/platform/core/src/emulator.cpp
    savetype_ids = [
