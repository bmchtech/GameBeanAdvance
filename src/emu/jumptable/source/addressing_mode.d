...
static void create_addressing_mode_2(bool is_register_offset, int shift_type, bool pre, bool up, bool byte_access, bool writeback, bool load)(IARM7TDMI cpu, Word opcode) {
    Reg rd = get_nth_bits(opcode, 12, 16);
    Reg rn = get_nth_bits(opcode, 16, 20);
    Word address = cpu.get_reg(rn);

    auto modify_address = (Word amount) {
        enum dir = up ? 1 : -1;
        return address + dir * amount;
    };
    ...
    static if (is_register_offset) {
        ...
        static if (pre) modify_address(offset);
    }
