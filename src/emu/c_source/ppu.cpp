...
void PPU::render_background_mode0(uint16_t bgcnt, uint16_t bghofs, uint16_t bgvofs) {
    uint16_t x            = dot             + bghofs;
    uint16_t y            = *memory->VCOUNT + bgvofs;

    uint32_t tile_base_address   = OFFSET_VRAM + get_nth_bits(bgcnt, 2,  4) * 0x4000;
    uint32_t screen_base_address = OFFSET_VRAM + get_nth_bits(bgcnt, 8, 13) * 0x800;
