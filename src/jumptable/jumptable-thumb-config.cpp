...
void run_0000SABC(uint16_t opcode) {
    ...
    if (shift == 0) { // if shift == 0, the cpu shifts by 32, which is the size of the register.
        ...
        @IF(!S) memory.regs[dest] = memory.regs[source];
    } else {
...
void run_00011010(uint16_t opcode) {
    ...
    int32_t rn = memory.regs[get_nth_bits(opcode, 3, 6)];
    int32_t rm = ~memory.regs[get_nth_bits(opcode, 6, 9)] + 1;
...
void run_00101ABC(uint16_t opcode) {
    ...
    // Signed carry formula = (A AND B) OR (~DEST AND (A XOR B)) - works for all add operations once tested
    set_flag_C(!(immediate_value > (uint32_t)old_rd_value));
...
void run_01001REG(uint16_t opcode) {
    ...
    uint32_t loc = (get_nth_bits(opcode, 0,  8) << 2) + ((*memory.pc + 2) & 0xFFFFFFFC);
...
void run_011BLOFS(uint16_t opcode) {
    ...
    @IF( B !L) memory.main[memory.regs[rn] + (immediate_value)] = memory.regs[rd] & 0xFF;
...
void run_11000REG(uint16_t opcode) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (get_nth_bit(register_list, i)) {
            ...
            // it has to be this way for when we writeback to memory.regs after the loop
            *start_address += 4;
            *(uint32_t*)(memory.main + ((*start_address - 4) & 0xFFFFFFFC)) = memory.regs[i];
        }
    }
}
